...
导表窗口固定读取 `RefDataSource/TryGameRefdataRes/v2` 正式源目录；该路径只读显示，不允许从临时或外部 Excel Root 发布正式产物。
...
2. 分开建立两套集合：窗口选中的 Excel 只决定本次执行哪些 cltabtoy/Language 导出并打印日志；持久化 manifest 固定扫描规范源目录中的全部业务/语言 Excel，并加入 `共用枚举结构体.xlsx`，形成与本次选择无关的完整 canonical 源快照。
...
- 完整 canonical Excel 集合与 `共用枚举结构体.xlsx` 在导出前、staging 验证、正式发布前及发布后都保持集合和 SHA256 一致，避免导出过程中新增/删除/修改源表却发布旧输出。
- 增量导出会先验证上一份正式 manifest v3、三份副本和四目录 payload；未选源表发生变化或公共依赖变化时拒绝启动 cltabtoy。
- manifest v3 能完整序列化，并按规范项目相对路径稳定排序记录完整源快照、表名、行数、工具版本，以及四个正式目录完整有效产物的相对路径和 SHA256。
